...
if ($_REQUEST["type"] == "add_savol_javob"){
    $savol = isset($_REQUEST["savol"]) ? $_REQUEST["savol"] : null;
    if (!$savol) {echo"savolni kiritishni unutdingiz!";exit;}

    $javob = isset($_REQUEST['javob']) ? $_REQUEST["javob"] : null;
    if (!$javob) {echo"javobni kiritishni unutdingiz!";exit;}

    $post_id = $db->insert("savol_javoblar", [
        "creator_user_id" => $user_id,
        "savol" => json_encode($savol, JSON_UNESCAPED_UNICODE),
        "javob" => json_encode($javob, JSON_UNESCAPED_UNICODE)
    ]);

    header('Location: savol_javoblar_list.php?page=1');
}
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title" id="basic-layout-colored-form-control">Savol-javob qo'shish</h4>
                        ...
                        <div class="card-body collapse in">
                            <div class="card-block">
                                <form action="add_savol_javob.php" method="POST" class="form" enctype="multipart/form-data">
                                    <input type="hidden" name="type" value="add_savol_javob" required>
                                    ...
                                    <? foreach ($db->in_array("SELECT * FROM langs_list") as $lang) { ?>
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            ...
                                            <textarea name="savol[<?=$lang["flag_icon"]?>]" class="form-control border-primary" placeholder="savol"></textarea>
                                        </div>
    
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            <label>javob <span class="flag-icon flag-icon-<?=$lang["flag_icon"]?>"></span></label>
                                            <textarea name="javob[<?=$lang["flag_icon"]?>]" class="form-control border-primary" placeholder="javob"></textarea>
                                        </div>
                                    <? } ?>
