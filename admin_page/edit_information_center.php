...
$information_center = $db->assoc("SELECT * FROM information_center WHERE id = ?", [$_REQUEST['id']]);
if (!$information_center["id"]) {echo"error (information_center not found)";exit;}

if ($_REQUEST['type'] == "edit_information_center"){
    $id = isset($_REQUEST['id']) ? $_REQUEST['id'] : null;
    ...
    [
        "id" => $information_center["id"]
    ]);

    header('Location: information_center.php?page='.$page);
}

if ($_REQUEST['type'] == "delete_information_center"){
    $db->delete("information_center", $information_center["id"]);
    delete_image($information_center["image_id"]);
    header('Location: information_center.php?page='.$page);
}
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title" id="basic-layout-colored-form-control">Axborot resusrs ma'lumotlarni tahrirlash</h4>
                        ...
                        <div class="card-body collapse in">
                            <div class="card-block">
                                <form action="edit_information_center.php" method="POST" class="form" enctype="multipart/form-data">
                                    <input type="hidden" name="type" value="edit_information_center" required>
                                    ...
                                    <? foreach ($db->in_array("SELECT * FROM langs_list") as $lang) { ?>
                                        ...
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            ...
                                            <textarea cols="80" id="editor" name="bio[<?=$lang["flag_icon"]?>]" class="form-control border-primary" rows="10" editor=""><?=lng($information_center["bio"], $lang["flag_icon"])?></textarea>
                                        </div>
                                        
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            <label>Foydalanish qoidalari <span class="flag-icon flag-icon-<?=$lang["flag_icon"]?>"></span></label>
                                            <textarea cols="80" id="editor" name="terms_use[<?=$lang["flag_icon"]?>]" class="form-control border-primary" rows="10" editor=""><?=lng($information_center["terms_use"], $lang["flag_icon"])?></textarea>
                                        </div>
    
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            <label>Ish vaqti <span class="flag-icon flag-icon-<?=$lang["flag_icon"]?>"></span></label>
                                            <textarea cols="80" id="editor" name="work_time[<?=$lang["flag_icon"]?>]" rows="10" class="form-control border-primary" editor=""><?=lng($information_center["work_time"], $lang["flag_icon"])?></textarea>
                                        </div>
                                    <? } ?>

                                    <div class="form-group">
                                        <label>Resurslar vebsite linki(https://niuedu.uz)</label>
                                        <input name="site" class="form-control border-primary" placeholder="vebsite" value="<?=$information_center["site"]?>"/>
                                    </div>
                                    
                                    <div class="form-group">
                                        <label>Elektron kutubxona linki(https://niuedu.uz)</label>
                                        <input name="library" class="form-control border-primary" placeholder="kutubxona" value="<?=$information_center["library"]?>"/>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="education_site" class="form-control border-primary" value="<?=$information_center["education_site"]?>"/>
                                    </div>
                                    
                                    <div class="form-group">
                                        <label>2 linki(https://niuedu.uz)</label>
                                        <input name="education_site2" class="form-control border-primary" value="<?=$information_center["education_site2"]?>"/>
                                    </div>
                                    
                                    <div class="form-group">
                                        <label>3 linki(https://niuedu.uz)</label>
                                        <input name="education_site3" class="form-control border-primary" value="<?=$information_center["education_site3"]?>"/>
                                    </div>
                                    
                                    <div class="form-group">
                                        <label>4 linki(https://niuedu.uz)</label>
                                        <input name="education_site4" class="form-control border-primary" value="<?=$information_center["education_site4"]?>"/>
                                    </div>
