...
if (!empty($_GET["to_course"])) {
    $query .= " AND to_course = '" . $_GET["to_course"] . "'";
}
...
$requests_count = $db->assoc("SELECT COUNT(id) FROM requests WHERE id > 0$query")["COUNT(id)"];
// exit($sql);
...
if (!empty($_GET["payment_method"])) {
    ...
    foreach ($requests as $request) {
        foreach ($request["payme"] as $payme) {
            if ($_GET["payment_method"] == "payme" && !empty($payme["id"])) {
                array_push($requests2, $request);
            }
        }

        if ($_GET["payment_method"] == "click" && !empty($request["click"]["id"])) {
            array_push($requests2, $request);
...
$directions_arr = $db->in_array("SELECT id, code, prefix, kunduzgi_narx_int, kechki_narx_int, sirtqi_narx_int, name FROM directions");
$directions = [];
...
if (empty($_GET["reg_type"])) {
    $suhbatdan_otganlar_soni = $db->assoc("SELECT COUNT(*) FROM requests WHERE suhbat = 1")["COUNT(*)"];

    $suhbatdan_otmaganlar_soni = $db->assoc("SELECT COUNT(*) FROM requests WHERE suhbat = 2")["COUNT(*)"];

    $ariza_topshirganlar_soni = $db->assoc("SELECT COUNT(*) FROM requests")["COUNT(*)"];
} else {
    $suhbatdan_otganlar_soni = $db->assoc("SELECT COUNT(*) FROM requests WHERE suhbat = 1 AND reg_type = ?", [ $_GET["reg_type"] ])["COUNT(*)"];

    $suhbatdan_otmaganlar_soni = $db->assoc("SELECT COUNT(*) FROM requests WHERE suhbat = 2 AND reg_type = ?", [ $_GET["reg_type"] ])["COUNT(*)"];

    $ariza_topshirganlar_soni = $db->assoc("SELECT COUNT(*) FROM requests WHERE reg_type = ?", [ $_GET["reg_type"] ])["COUNT(*)"];
}
...
$firms = [];
$firms_arr = $db->in_array("SELECT id, first_name, last_name FROM firms");
foreach ($firms_arr as $firm_arr) {
    $firms[$firm_arr["id"]] = $firm_arr;
}
                ...
                <div class="col-xs-12">
                    <div class="card">
                        <div class="card-header" id="header">
                            <h4 class="card-title" title="<?=$sql?>">Arizalar ro'yxati (<?=$requests_count?> ta)</h4>
                                ...
                                <div class="row">
                                    <div class="col-xl-4 col-lg-4 col-sm-6 col-12">
                                        ...
                                        <select name="direction_id" class="form-control" id="direction_id">
                                            ...
                                            <? foreach ($db->in_array("SELECT * FROM directions") as $direction_arr) { ?>
                                                <option value="<?=$direction_arr["id"]?>" <?=($_GET["direction_id"] && $direction_arr["id"] == $_GET["direction_id"] ? 'selected=""' : '')?>><?=lng($direction_arr["short_name"], "uz")?></option>
                                            <? } ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-xl-2 col-lg-2 col-sm-6 col-12">
                                        ...
                                        <select name="learn_type" class="form-control" id="learn_type">
                                            ...
                                            <option value="Kechki" <?=($_GET["learn_type"] && $_GET["learn_type"] == "Kechki" ? 'selected=""' : '')?>>Kechki</option>
                                    ...
                                    <div class="col-xl-2 col-lg-2 col-sm-6 col-12">
                                        ...
                                        <select name="reg_type" class="form-control" id="reg_type">
                                            ...
                                            <option value="ikkinchi-mutaxassislik" <?=($_GET["reg_type"] && "ikkinchi-mutaxassislik" == $_GET["reg_type"] ? 'selected=""' : '')?>>Ikkinchi mutaxassislik</option>
                                    ...
                                    <div class="col-xl-2 col-lg-2 col-sm-6 col-12" style="margin-top:32px;margin-left:25px;">
                                        <a href="/<?=$url2[0]?>/requests_list_export_to_excel.php<?=($_GET ? "?".htmlspecialchars(http_build_query($_GET)) : "")?>" class="btn btn-success" id="submit-date"><i class="icon-file5"></i> Barcha arizalarni olish (EXCEL)</a>
                                    </div>
                                    ...
                                    <div class="col-xl-4 col-lg-4 col-sm-6 col-12">
                                        ...
                                        <select name="firm_id" class="form-control" id="firm_id">
                                            ...
                                            <? foreach ($firms as $firm) { ?>
                                                <option value="<?=$firm["id"]?>" <?=($_GET["firm_id"] && $firm["id"] == $_GET["firm_id"] ? 'selected=""' : '')?>><?=$firm["first_name"] . " " . $firm["last_name"]?></option>
                                            <? } ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-xl-4 col-lg-4 col-sm-6 col-12">
                                        ...
                                        <select name="to_course" class="form-control" id="to_course">
                                            <option value="" <?=(!$_GET["to_course"] || "" == $_GET["to_course"] ? 'selected=""' : '')?>>Barcha kurslar</option>

                                            <? foreach ($courses as $course) { ?>
                                                <option value="<?=$course["name"]?>" <?=($_GET["to_course"] && $course["name"] == $_GET["to_course"] ? 'selected=""' : '')?>><?=$course["name"]?></option>
                                            <? } ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-xl-4 col-lg-4 col-sm-6 col-12">
                                        <label>5-yil staj</label>
                                        <select name="staj" class="form-control" id="staj">
                                            <option value="">Barchasi</option>

                                            <option value="1" <?=($_REQUEST["staj"] == "1" ? 'selected=""' : "")?>>5-yil staj</option>
                                        </select>
                                    </div>
                                </div>
                                            ...
                                            foreach ($requests as $request){
                                                ...
                                                echo '<tr class="bg-white">';
                                                    ...
                                                    if (!empty($quiz_user_block["id"])) {
                                                        ...
                                                        // echo '</td>';
                                                        echo '<td>';
                                                            echo '<a href="edit_request_ball.php?request_id='.$request['id'].'" class="tag tag-default tag-warning text-white bg-success">'.$quiz_user_block["ball"].'-ball</a>';
                                                        echo '</td>';
                                                    } else {
                                                        echo '<td>';
                                                            echo '<a href="edit_request_ball.php?request_id='.$request['id'].'" class="tag tag-default tag-warning text-white bg-danger">Test tashkillashtirib bo\'lmaydi</a>';
                                                        echo '</td>';
                                                        // echo '<td>';
                                                    ...
                                                    if (!empty($request["click"]["id"])) {
                                                        ...
                                                        echo '</td>';
                                                    } else if (!empty($request["payme"][0]["id"])) {
                                                        echo '<td>';
                                                        foreach ($request["payme"] as $payme) {
                                                            // if ($payme["hide"] == 1) {
                                                            //     echo '<a href="javascript:void(0)" class="tag tag-default tag-warning text-white bg-danger">kutilmoqda<br><small>('.number_format(($payme["amount"] / 100), 0).' UZS)</small><br><small>qayta imtihon uchun</small></a>';
                                                            // } else {
                                                            // }
                                                            echo '<a href="javascript:void(0)" class="tag tag-default tag-warning text-white bg-success">qabul qilingan<br><small>('.number_format(($payme["amount"] / 100), 0).' UZS)</small><br><small>payme</small></a>';
                                                        }
                                                        echo '</td>';
                                                    } else if (!empty($request["kassa"]["id"])) {
                                                        // echo '<td>';
                                                    ...
                                                    echo '<td>';
                                                        echo '<div class="btn-group mr-1 mb-1">';
                                                            echo '<button type="button" class="btn btn-success btn-min-width dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'.$firms[$request["firm_id"]]["first_name"]." ".$firms[$request["firm_id"]]["last_name"].'</button>';
                                                            echo '<div class="dropdown-menu">';
                                                            ...
                                                            $key = 0;
                                                            foreach ($firms as $firm) {
                                                                $_GET["type"] = "change_firm";
                                                                $_GET["request_id"] = $request["id"];
                                                                $_GET["firm_id"] = $firm["id"];

                                                                $ajax_link_3 = "$edit_request".($_GET ? "?".htmlspecialchars(http_build_query($_GET)) : "");

                                                                echo '<a data-ajax-href="'.$edit_request.'?type=change_firm&firm_id='.$firm["id"].'&request_id='.$request["id"].'&page='.$_GET["page"].'" class="dropdown-item">'.($request["firm_id"] == $firm["id"] ? "✅" : "").' '.$firm["first_name"] . " " . $firm["last_name"] .'</a>';
                                                    ...
                                                    echo '<td>';
                                                        echo '<div class="btn-group mr-1 mb-1">';
                                                            echo '<button type="button" class="btn btn-success btn-min-width dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'.$request["to_course"].'</button>';
                                                            echo '<div class="dropdown-menu">';

                                                            $_GET["type"] = "change_course";
                                                            $_GET["request_id"] = $request["id"];
                                                            $_GET["to_course"] = "";

                                                            $ajax_link_1 = "$edit_request".($_GET ? "?".htmlspecialchars(http_build_query($_GET)) : "");

                                                            echo '<a data-ajax-href="'.$ajax_link_1.'" class="dropdown-item">Kursdan chiqarish</a>';
                                                            echo '<div class="dropdown-divider"></div>';
                                                            ...
                                                            foreach ($courses as $course) {
                                                                $_GET["type"] = "change_course";
                                                                $_GET["request_id"] = $request["id"];
                                                                $_GET["to_course"] = $course["name"];

                                                                $ajax_link = "$edit_request".($_GET ? "?".htmlspecialchars(http_build_query($_GET)) : "");

                                                                echo '<a data-ajax-href="'.$ajax_link.'" class="dropdown-item">'.($request["to_course"] == $course["name"] ? "✅" : "").' '.$course["name"].'</a>';
                                                    ...
                                                    if ($request["staj_file_id"] > 0) array_push($files_arr, $request["staj_file_id"]);
                                                    ...
                                                    if (!empty($request["shartnoma_date"])) {
                                                        ...
                                                        echo '<td>'.($request["shartnoma_amount"] ? '<b>'.number_format($request["shartnoma_amount"]) : '<b>'.number_format($contract_amount).'</b>').'</td>';
                                                    } else {
                                                    ...
                                                    echo '<td>'.($payed_amount ? '<b>'.number_format($payed_amount).' </b>' : '<b class="text-danger">to\'lamagan</b>').'</td>';

                                                    // echo '<td>'.($request["shartnoma_number"] ? '<b>'.$request["shartnoma_number"].'</b>' : '<b class="text-danger">kiritilmagan</b>').'</td>';

                                                    if (!empty($request["shartnoma_date"])) {
                                                        $shartnoma_code =  "$direction[prefix]-$request[code]/" . ($request["learn_type"] == "Sirtqi" ? "S" : "K");

                                                        echo '<td>'.'<b>'.$shartnoma_code.'</b>'.'</td>';
                                                    } else {
                                                    ...
                                                    echo '<td>';
                                                        echo '<a href="requests_list.php?code='.$request['code'].'&send_sms=1" target="_blank" class="tag tag-default tag-warning text-white bg-success">sms yuborish</a>';
                                                    echo '</td>';
                            ...
                            <? if (!$disable_pagination && !$_GET["page_count"]) { ?>
                                ...
                                <div class="text-xs-center mb-3" id="pagination-wrapper">
                                    <nav aria-label="Page navigation">
                                        ...
                                        <ul class="pagination">
                                            <?
                                            $count = (int)$db->assoc("SELECT COUNT(*) FROM requests WHERE id > 0$query ORDER BY id ASC")["COUNT(*)"] / $page_count;
...
<script>
    ...
    $("#show-all").on("click", function(){
        ...
        url = url + "&to_course=" + findGetParameter("to_course");
        ...
        url = url + "&staj=" + findGetParameter("staj");
    ...
    $("#to_course").on("change", function(){
        ...
        url = url + "&to_date=" + findGetParameter("to_date");
        url = url + "&firm_id=" + findGetParameter("firm_id");
        url = url + "&to_course=" + $("#to_course").val();
        url = url + "&document_type_id=" + findGetParameter("document_type_id");
        url = url + "&contract_type_id=" + findGetParameter("contract_type_id");
        url = url + "&contract_payment=" + findGetParameter("contract_payment");
        url = url + "&shartnoma_date=" + findGetParameter("shartnoma_date");
