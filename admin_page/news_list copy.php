...
<div class="app-content content container-fluid">
    ...
    $_GET["page"] = 1;
                                          ...
                                          $news = $db->in_array("SELECT * FROM news ORDER BY id ASC LIMIT $page_start, $page_count");
                                          ...
                                          foreach ($news as $news){
                                              ...
                                              echo '<tr>';
                                                  ...
                                                  echo '<td>'.substr($news['mini_text'],0,30).'</td>';
                                                  echo '<td>'.$news['created_date'].'</td>';
                                                  echo '<td>';
                                                    echo '<a href="edit_news.php?news_id='.$news['id'].'&page='.$_GET["page"].'" class="tag tag-default tag-warning text-white bg-success">taxrirlash</a>';
                                                  echo '</td>';
                                                  echo '<td>';
                                                    echo '<a href="edit_news.php?type=delete_news&news_id='.$news['id'].'&page='.$_GET["page"].'" class="tag tag-default tag-warning text-white bg-danger">o`chirish</a>';
                                                echo '</td>';
                                              echo '</tr>';
                                          }
                                        ?>
                            ...
                            <div class="text-xs-center mb-3">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination">
                                        <?
                                        $count = (int)$db->assoc("SELECT COUNT(*) FROM news")["COUNT(*)"] / $page_count;

                                        if (gettype($count) == "double") $count = (int)($count + 1);
                        
                                        if ($page != 1){
                                          echo '<li class="page-item">
                                                    <a class="page-link" href="news_list.php?page='.($page-1).'">
                                        ...
                                        for ($i = 0; $i <= $count; $i++) {
                                            if ($i == 1 || $i == $count || $i >= $page && $i <= $page + ($max - 1)) {
                                                echo '<li class="page-item '.($page == $i ? "active" : "").'"><a href="news_list.php?page='.$i.'" class="page-link">'.$i.'</a></li>';
                                            }
                                        }
