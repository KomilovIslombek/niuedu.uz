...
if ($_REQUEST['type'] == "add_media"){
    $iframe = isset($_REQUEST["iframe"]) ? $_REQUEST["iframe"] : null;
    if (!$iframe) {echo"mediani kiritishni unutdingiz!";exit;}

    $name = isset($_REQUEST["name"]) ? $_REQUEST["name"] : null;
    if (!$name) {echo"[name]ni kiritishni unutdingiz!";exit;}
    
    $media_id = $db->insert("medias", [
        "creator_user_id" => $user_id,
        "iframe" => $iframe,
        "name" => json_encode($name, JSON_UNESCAPED_UNICODE)
    ]);

    header('Location: medias_list.php?page=1');
}
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title" id="basic-layout-colored-form-control">Media qo'shish</h4>
                        ...
                        <div class="card-body collapse in">
                            <div class="card-block">
                                <form action="add_media.php" method="POST" class="form" enctype="multipart/form-data">
                                    <input type="hidden" name="type" value="add_media" required>

                                    <div class="form-group">
                                        <label>media (iframe youtube)</span></label>
                                        <textarea type="text" name="iframe" class="form-control border-primary" placeholder="iframe" style="min-height:150px"></textarea>
                                    </div>
                                    ...
                                    <? foreach ($db->in_array("SELECT * FROM langs_list") as $lang) { ?>
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            ...
                                            <textarea type="text" name="name[<?=$lang["flag_icon"]?>]" class="form-control border-primary" placeholder="nomi"></textarea>
                                        </div>
                                    <? } ?>
