...
$mission = $db->assoc("SELECT * FROM mission WHERE id = ?", [$_REQUEST['id']]);
if (!$mission["id"]) {echo"error (mission not found)";exit;}

if ($_REQUEST['type'] == "edit_mission"){
    $id = isset($_REQUEST['id']) ? $_REQUEST['id'] : null;
...
if ($_REQUEST['type'] == "delete_mission"){
    $db->delete("mission", $mission["id"]);
    delete_image($mission["image_id"]);
    header('Location: mission.php?');
}
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title" id="basic-layout-colored-form-control">Missiyani tahrirlash</h4>
                        ...
                        <div class="card-body collapse in">
                            <div class="card-block">
                                <form action="edit_mission.php" method="POST" class="form" enctype="multipart/form-data">
                                    <input type="hidden" name="type" value="edit_mission" required>
                                    ...
                                    <? foreach ($db->in_array("SELECT * FROM langs_list") as $lang) { ?>
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            <label>Missiyamiz <span class="flag-icon flag-icon-<?=$lang["flag_icon"]?>"></span></label>
                                            <textarea cols="80" id="editor" name="html[<?=$lang["flag_icon"]?>]" rows="10" class="form-control border-primary" editor=""><?=lng($mission["html"], $lang["flag_icon"])?></textarea>
                                        </div>
                                    <? } ?>
