...
if ($_REQUEST['type'] == "add_council"){

    $name = isset($_REQUEST['name']) ? $_REQUEST['name'] : null;
    if (!$name) {echo"nomini kiritishni unutdingiz!";exit;}
    
    $council_id = $db->insert("council", [
        "creator_user_id" => $user_id,
        "name" => json_encode($name, JSON_UNESCAPED_UNICODE),
    ]);

    header('Location: council.php?page=1');
}
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title" id="basic-layout-colored-form-control">Ilmiy kengash qo'shish</h4>
                        ...
                        <div class="card-body collapse in">
                            <div class="card-block">
                                <form action="add_council.php" method="POST" class="form" enctype="multipart/form-data">
                                    <input type="hidden" name="type" value="add_council" required>
                                    ...
                                    <? foreach ($db->in_array("SELECT * FROM langs_list") as $lang) { ?>
                                        <div class="form-group" form-lang="<?=$lang["flag_icon"]?>" style="<?=($lang["flag_icon"] != "uz" ? "display:none" : "")?>">
                                            ...
                                            <textarea name="name[<?=$lang["flag_icon"]?>]" class="form-control border-primary" placeholder="nomi"></textarea>
                                        </div>
