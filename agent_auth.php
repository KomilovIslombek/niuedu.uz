...
if ($_POST["submit"]) {
    ...
    if (!$pass) {
        $pass_error = "Passport seriyasi hamda kiritishni unutdingiz!";
    }
    ...
    if ($login && $pass) {
        ...
        if (empty($user["id"])) {
            $agent = $db->assoc("SELECT * FROM firms WHERE phone_1 = ?", [ $login ]);
            
            if (empty($agent["id"])) {
                $error = "bunday ID raqamli agent topilmadi";
            } else if (str_replace(" ", "", $pass) != str_replace(" ", "", $agent["passport_serial_number"])) {
                $error = "passport seriya xato";
            } else if (!empty($agent["id"])) {
                        ...
                        <div class="wrapper wrapper2 mt-5">
                            <form method="POST" id="forgotpsd" class="card-body">
                                ...
                                <div class="was-validated">
                                    ...
                                    <div class="invalid-feedback">
                                        <?=($pass_error ? $pass_error : "iltimos Passport seriyangizni kiriting")?>
                                    </div>
                                </div>
